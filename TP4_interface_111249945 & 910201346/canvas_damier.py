...
class CanvasDamier(Canvas):
    ...
    def __init__(self, parent, damier, n_pixels_par_case=60):
        ...
        Args:
            ...
            couleur : choix de couleur par l'utilisateur

        """
        # fenêtre informant l'utilisateur qu'il doit choisir une couleur pour son damier
        messagebox.showinfo("Information", "Choississez une couleur pour votre damier")

        # l'utilisateur choisit la couleur pour son damier
        self.couleur = colorchooser.askcolor()[1]
    ...
    def dessiner_cases(self):
        ...
        for i in range(self.damier.n_lignes):
            for j in range(self.damier.n_colonnes):
                ...
                # On détermine la couleur du damier en passant le choix de couleur de l'utilisateur
                if (i + j) % 2 == 0:
